...
This card applies to `mechanics/tos-bridge/docs/` and all descendant source documents.

## Role

`mechanics/tos-bridge/docs/` holds detailed center-source doctrine, models, waves, stop-lines,
handoffs, packets, or support notes for the `ToS bridge` mechanic. The package `README.md` remains
the entry card; this docs directory holds the deeper material.

## Read before editing

Read root `AGENTS.md`, `mechanics/AGENTS.md`, `mechanics/tos-bridge/AGENTS.md`,
`mechanics/tos-bridge/README.md`, and the specific source document you are changing. If a generated
...
python scripts/validate_mechanic_landing_logs.py --mechanic tos-bridge
python -m pytest -q
```

...
| `ORQ-CHECKPOINT-SKILLS-001` | `aoa-skills` | `landed` | `P0` | Checkpoint note protocol and closeout bridge skill | Skill-owned checkpoint note contract and explicit closeout bridge remain lower-authority than reviewed closeout. | Bridge execution evidence routes to `aoa-evals` when verdict claims are made. |
...
| `ORQ-CHECKPOINT-ROUTING-001` | `aoa-sdk` | `requested` | `P2` | Re-entry hints | Routing hints point back to source-owned checkpoint and return surfaces without interpreting checkpoint meaning. | Route behavior can be checked through evals if public. |

...
def test_generated_registry_shape():
    reg = json.loads((ROOT / 'mechanics/agon/generated/agon_epistemic_agon_registry.min.json').read_text(encoding='utf-8'))
    assert reg['wave'] == 'XV'
...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics/agon/scripts/build_agon_epistemic_agon_registry.py'), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics/agon/scripts/validate_agon_epistemic_agon.py')], cwd=ROOT).returncode == 0

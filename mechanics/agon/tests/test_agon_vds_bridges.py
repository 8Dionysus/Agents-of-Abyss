...
def test_vds_registry_is_build_clean():
    r=subprocess.run([sys.executable,str(ROOT / "mechanics" / "agon" / "scripts"/'build_agon_vds_bridge_registry.py'),'--check'],cwd=ROOT,text=True,capture_output=True)
    assert r.returncode==0, r.stdout+r.stderr
def test_vds_registry_validates():
    data=_load(ROOT / "mechanics" / "agon" / "scripts"/'validate_agon_vds_bridges.py').validate()

...
def test_arena_session_registry_is_up_to_date():
    result = run_script('mechanics/agon/parts/packet-arena/scripts/build_agon_arena_session_model_registry.py', '--check')
    assert result.returncode == 0, result.stderr + result.stdout


def test_arena_session_models_validate():
    result = run_script('mechanics/agon/parts/packet-arena/scripts/validate_agon_arena_session_models.py')
    assert result.returncode == 0, result.stderr + result.stdout


def test_registry_remains_pre_protocol():
    registry = json.loads((ROOT / "mechanics" / "agon" / "parts" / "packet-arena" / "generated" / "agon_arena_session_model_registry.min.json").read_text(encoding='utf-8'))

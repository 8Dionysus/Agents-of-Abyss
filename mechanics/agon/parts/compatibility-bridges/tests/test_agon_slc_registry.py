...
ENTRY_SCHEMA = ROOT / "mechanics" / "agon" / "parts" / "compatibility-bridges" / "schemas" / "agon-school-lineage-campaign.schema.json"
REGISTRY_SCHEMA = ROOT / "mechanics" / "agon" / "parts" / "compatibility-bridges" / "schemas" / "agon-school-lineage-campaign-registry.schema.json"
...
def test_slc_bridge_registry_shape():
    reg = load(OUT)
    assert reg['lineage_ref'] == 'school-lineage-campaign'
    assert reg['lineage_title'] == 'Schools / Lineages / Campaigns'
    ...
    for item in reg[ITEM_KEY]:
        assert item['lineage_ref'] == 'school-lineage-campaign'
...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(ROOT / "mechanics" / "agon" / "parts" / "compatibility-bridges" / "scripts" / "build_agon_slc_registry.py"), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(ROOT / "mechanics" / "agon" / "parts" / "compatibility-bridges" / "scripts" / "validate_agon_slc_registry.py")], cwd=ROOT).returncode == 0

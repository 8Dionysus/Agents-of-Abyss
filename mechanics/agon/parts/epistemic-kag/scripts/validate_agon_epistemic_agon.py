...
from build_agon_epistemic_agon_registry import (  # noqa: E402
    BuildError,
    ITEM_KEY,
    MOVE_EXTENSIONS_KEY,
    OUT,
...
ITEM_SCHEMA = ROOT / "mechanics" / "agon" / "parts" / "epistemic-kag" / "schemas" / "agon-epistemic-agon.schema.json"
REGISTRY_SCHEMA = ROOT / "mechanics" / "agon" / "parts" / "epistemic-kag" / "schemas" / "agon-epistemic-agon-registry.schema.json"
...
def validate_item(item: Any, item_validator: jsonschema.Draft202012Validator) -> str | None:
    ...
    if item.get("owner_repo") != "Agents-of-Abyss":
        return f"{key} owner_repo must be Agents-of-Abyss"
...
def validate() -> int:
    ...
    move_extensions = data.get(MOVE_EXTENSIONS_KEY)
    if not isinstance(move_extensions, list) or not all(isinstance(name, str) and name for name in move_extensions):
        return fail(f"source {MOVE_EXTENSIONS_KEY} must be a non-empty string array")
    if len(move_extensions) != EXPECTED_MOVE_EXTENSION_COUNT:
        return fail(
            f"expected {EXPECTED_MOVE_EXTENSION_COUNT} items in {MOVE_EXTENSIONS_KEY}, got {len(move_extensions)}"
        )

    item_validator = make_item_validator()
    seen: set[str] = set()
    component_move_extensions: set[str] = set()
    for item in items:
        ...
        seen.add(key)
        component_move_extensions.update(item["move_extensions"])

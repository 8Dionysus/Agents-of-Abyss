...
def test_builder_check_and_validator():
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics/agon/parts/epistemic-kag/scripts/build_agon_kag_promotion_path_registry.py'), '--check'], cwd=ROOT).returncode == 0
    assert subprocess.run([sys.executable, str(ROOT / 'mechanics/agon/parts/epistemic-kag/scripts/validate_agon_kag_promotion_path_registry.py')], cwd=ROOT).returncode == 0

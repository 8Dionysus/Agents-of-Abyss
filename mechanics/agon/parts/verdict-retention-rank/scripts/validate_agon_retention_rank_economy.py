...
ROOT = _repo_root()
REGISTRY = ROOT / "mechanics/agon/parts/verdict-retention-rank/generated/agon_retention_rank_economy_registry.min.json"
SCHEMA = ROOT / "mechanics/agon/parts/verdict-retention-rank/schemas/agon-retention-rank-economy-registry.schema.json"
BUILDER = ROOT / "mechanics/agon/parts/verdict-retention-rank/scripts/build_agon_retention_rank_economy_registry.py"
...
def expected_registry() -> dict:
    spec = importlib.util.spec_from_file_location("agon_retention_rank_builder", BUILDER)
...
def main() -> int:
    ...
    if data.get("live_protocol") is not EXPECTED_LIVE_PROTOCOL:
        raise SystemExit("retention-rank route registries must remain non-live protocol surfaces")

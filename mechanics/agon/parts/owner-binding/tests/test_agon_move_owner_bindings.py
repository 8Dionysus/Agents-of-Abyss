...
def load_builder():
    path = ROOT / "mechanics" / "agon" / "parts" / "owner-binding" / "scripts" / "build_agon_move_owner_binding_registry.py"
...
def test_agon_move_owner_binding_registry_is_current() -> None:
    result = subprocess.run(
        [sys.executable, "mechanics/agon/parts/owner-binding/scripts/build_agon_move_owner_binding_registry.py", "--check"],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
...
def test_agon_move_owner_binding_validator_cli_is_current() -> None:
    result = subprocess.run(
        [sys.executable, "mechanics/agon/parts/owner-binding/scripts/validate_agon_move_owner_bindings.py"],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
...
def test_owner_binding_validator_rejects_non_list_fields() -> None:
    ...
    config = json.loads((ROOT / "mechanics" / "agon" / "parts" / "owner-binding" / "config" / "agon_move_owner_bindings.seed.json").read_text(encoding="utf-8"))

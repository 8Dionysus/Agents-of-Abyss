...
def load_builder():
    path = ROOT / "mechanics" / "agon" / "parts" / "lawful-move-grammar" / "scripts" / "build_agon_lawful_move_registry.py"
...
def test_lawful_move_registry_is_current() -> None:
    result = subprocess.run(
        [sys.executable, "mechanics/agon/parts/lawful-move-grammar/scripts/build_agon_lawful_move_registry.py", "--check"],
        cwd=ROOT,
        text=True,
        capture_output=True,
    )
...
def test_builder_rejects_unknown_move_keys() -> None:
    ...
    move = next(
        item
        for item in json.loads((ROOT / "mechanics" / "agon" / "parts" / "lawful-move-grammar" / "config" / "agon_lawful_moves.seed.json").read_text(encoding="utf-8"))["moves"]

# Distillation Roadmap

## Current contour

Distillation is landed as center law and route grammar. It defines how heavy
material becomes active form without losing provenance or inflating authority.

## When the time comes

- Add receipt schema support when multiple mechanics need shared receipt IDs
  rather than local receipt maps.
- Add owner-local proof bundles in `aoa-evals` when provenance-preservation
  checks become reusable beyond current compost and package validators.
- Add SDK helpers only after at least two mechanics need the same mechanical
  receipt or provenance workflow.
- Add runtime-pack examples only after `aoa-agents`, `aoa-memo`, and
  `abyss-stack` agree on the boundary.

## Do not rush

- Do not centralize every local distillation validator too early.
- Do not move raw source histories from other mechanics into this package.
- Do not treat this mechanic as a universal summarizer.

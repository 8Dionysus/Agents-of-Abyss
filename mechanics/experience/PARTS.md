...
|---|---|---|
| [Capture Kernel](parts/capture-kernel/README.md) | friction record, candidate packet, owner route, memory gate, and inert projection boundary | `aoa-memo` for memory objects, `aoa-evals` for proof, `aoa-sdk` for live route behavior |
...
| [Governance Polis](parts/governance-polis/README.md) | decision table, C25 hash-bound sole-operator envelope, policy registry route, appeal or stay route, precedent index hint, and constitution-runtime stop-line | `Agents-of-Abyss` owns the constitutional envelope; named owners retain payload meaning, validation, effects, runtime, memory, routing, and proof |
...
| [Office Operations](parts/office-operations/README.md) | office contour, emergency-stay-only operator artifact, role-pair handoff, assistant invariant, and owner-local office request | `aoa-agents` owns actor and handoff posture; future owner-local offices own live behavior; generic operator decisions route to Governance Polis C25 |
| [Service Mesh](parts/service-mesh/README.md) | service operation drill, service law stop-line, compatibility route, and runtime-owner request | `abyss-stack` owns service runtime; `aoa-sdk` owns live dispatch behavior after owner gates |
| [Continuity Context](parts/continuity-context/README.md) | context route hint, continuity weave boundary, replay audit route, memory handoff, and re-entry stop-line | `aoa-memo` owns memory objects; `aoa-sdk` owns live context router behavior; `aoa-evals` owns proof |

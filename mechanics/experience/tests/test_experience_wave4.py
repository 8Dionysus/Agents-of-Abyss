...
def load_validator():
    path = ROOT / "scripts" / "validate_experience_wave4.py"
...
def load_example() -> dict[str, object]:
    return json.loads((ROOT / "examples" / "experience_wave4_polis_constitution.example.json").read_text())


def test_experience_wave4_validator_passes() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/validate_experience_wave4.py"],
        cwd=ROOT,
        text=True,
        capture_output=True,
    )

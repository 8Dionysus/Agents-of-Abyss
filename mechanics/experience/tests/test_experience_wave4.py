...
def load_validator():
    path = ROOT / "mechanics" / "experience" / "scripts" / "validate_experience_wave4.py"
...
def load_example() -> dict[str, object]:
    return json.loads((ROOT / "mechanics" / "experience" / "examples" / "experience_wave4_polis_constitution.example.json").read_text())


def test_experience_wave4_validator_passes() -> None:
    result = subprocess.run(
        [sys.executable, "mechanics/experience/scripts/validate_experience_wave4.py"],
        cwd=ROOT,
        text=True,
        capture_output=True,
    )

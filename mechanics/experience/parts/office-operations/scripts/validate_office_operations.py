...
def validate_office_contract(flow: dict[str, Any]) -> None:
    ...
    primary = set(
        str(item)
        for item in require_list(
            office.get("required_primary_offices"), "required_primary_offices"
        )
    )
    missing_primary = REQUIRED_PRIMARY_OFFICES.difference(primary)
    if missing_primary:
        fail(
            "office_contract.required_primary_offices is missing: "
            + ", ".join(sorted(missing_primary))
        )
    non_primary = set(
        str(item)
        for item in require_list(
            office.get("non_primary_until_promoted"), "non_primary_until_promoted"
        )
    )
    missing_non_primary = NON_PRIMARY_UNTIL_PROMOTED.difference(non_primary)
    if missing_non_primary:
        fail(
            "office_contract.non_primary_until_promoted is missing: "
            + ", ".join(sorted(missing_non_primary))
        )

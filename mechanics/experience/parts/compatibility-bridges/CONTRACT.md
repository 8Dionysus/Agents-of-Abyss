# Compatibility Bridges Contract

## Center boundary

This part owns cross-mechanic compatibility, Agon/Experience seams, ToS candidate boundaries, KAG promotion gates, cross-kind conflict routes, and bridge-only language.

## Stronger owner split

the crossed mechanic or owner repository keeps authority; Experience only records center-side compatibility constraints.

## Allowed outputs

- compatibility note, bridge stop-line, cross-kind conflict route, ToS or KAG handoff, and owner split reminder.
- Center-side request packets, compatibility notes, and stop-lines.
- Inert projections that explicitly name the owner-local gate required before activation.

## Must not claim

- Live workspace runtime.
- Hidden memory sovereignty.
- Owner-local activation or acceptance.
- Proof verdicts before the proof owner lands evidence.
- `aoa-kag` projections as source-authored meaning.
- ToS-authored meaning or ToS canon from an AoA center bridge.

## Next route

When this part needs operational landing, update `OWNER_REQUESTS.md` and `mechanics/owner-request-queue.json`, then route to the stronger owner named above. Keep historical-source detail out of active part docs and keep only the distilled contract here.

...
INDEX_CONTRACT_SCHEMA = "agents_of_abyss_decision_index_contract_v1"
DECISION_ID_PREFIX = "AOA-CENTER-D"
PATH_MODE = "full_canonical_id_filename"
SOURCE_RECORDS = "docs/decisions/AOA-CENTER-D-*.md"
GENERATED_BY = "scripts/docs_districts/generate_decision_indexes.py"
GENERATED_INDEX_PATHS = (
    INDEXES_DIR / "README.md",
    INDEXES_DIR / "by-number.md",
    INDEXES_DIR / "by-date.md",
    INDEXES_DIR / "by-surface.md",
    INDEXES_DIR / "by-center-facet.md",
    INDEXES_DIR / "by-mechanic.md",
    INDEXES_DIR / "by-guard.md",
)

DECISION_ID_RE = re.compile(r"^- Decision ID: (AOA-CENTER-D-(\d{4}))$", re.MULTILINE)
FULL_ID_FILENAME_RE = re.compile(r"^(AOA-CENTER-D-(\d{4}))-.+\.md$")
DATE_VALUE_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$")
STATUS_RE = re.compile(r"^## Status\s*\n+(?P<status>[A-Za-z][A-Za-z0-9_-]*)\.\s*$", re.MULTILINE)
...
def parse_decision_id(text: str, *, path: Path) -> tuple[str, int]:
    ...
    if not match:
        raise ValueError(f"{path.as_posix()} is missing '- Decision ID: AOA-CENTER-D-####'")
    return match.group(1), int(match.group(2))


def parse_status(text: str, *, path: Path) -> str:
    match = STATUS_RE.search(text)
    if not match:
        raise ValueError(f"{path.as_posix()} is missing '## Status' with a one-word value")
...
def parse_index_metadata(text: str, *, path: Path) -> dict[str, str]:
    ...
    if missing:
        raise ValueError(f"{path.as_posix()} index metadata is missing: {', '.join(missing)}")
    return metadata
...
def load_decision_record(path: Path, *, repo_root: Path) -> DecisionRecord:
    ...
    return DecisionRecord(
        ...
        path=relative_path,
        original_date=parse_original_date(metadata, path=relative_path),
        status=parse_status(text, path=relative_path),
        surface_classes=split_metadata_value(metadata["surface classes"]),
        center_facets=split_metadata_value(metadata["center facets"]),
...
def collect_decision_records(repo_root: Path) -> tuple[list[DecisionRecord], list[tuple[str, str]]]:
    ...
    for path in sorted(
        ...
        if not filename_match:
            issues.append((record.repo_path, "decision path must use the full canonical ID filename format"))
        elif filename_match.group(1) != record.decision_id:
            issues.append((record.repo_path, "decision path canonical ID must match the note Decision ID"))
        elif int(filename_match.group(2)) != record.number:
            issues.append((record.repo_path, "decision path number must match the note Decision ID number"))
...
def render_generated_notice() -> str:
    return "<!-- Generated by scripts/docs_districts/generate_decision_indexes.py; do not edit by hand. -->\n\n"
...
def render_indexes_readme() -> str:
    return (
        ...
        + "These files are generated read models from center decision-note `Index Metadata`.\n"
        ...
        + "- [By center facet](by-center-facet.md)\n"
...
def render_by_number(records: Sequence[DecisionRecord]) -> str:
    ...
    for record in records:
        lines.append(
            "| {decision_id} | {date} | {status} | [{title}]({link}) | `{path}` | {surfaces} | {facets} | {parents} | {guards} | {posture} |".format(
                ...
                facets=", ".join(record.center_facets) or "none",
...
def render_by_date(records: Sequence[DecisionRecord]) -> str:
    ...
    for date in sorted({record.original_date for record in records}):
        lines.extend([f"## {date}", ""])
        for record in records:
            if record.original_date == date:
...
def render_index_files(records: Sequence[DecisionRecord]) -> dict[Path, str]:
    return {
        ...
        INDEXES_DIR / "by-center-facet.md": render_grouped_index(
            title="Decisions By Center Facet",
            records=records,
            attribute="center_facets",
            preferred_order=CENTER_FACET_ORDER,
        ),
...
def validate_index_contract_payload(contract: dict[str, object]) -> list[tuple[str, str]]:
    ...
    if contract.get("schema_version") != INDEX_CONTRACT_SCHEMA:
        issues.append((INDEX_CONTRACT_PATH.as_posix(), f"schema_version must be {INDEX_CONTRACT_SCHEMA}"))
    ...
    if contract.get("generated_indexes") != expected_indexes:
        issues.append((INDEX_CONTRACT_PATH.as_posix(), "generated_indexes must match the decision index read-model set"))
    if contract.get("fields") != expected_contract_fields():
        issues.append((INDEX_CONTRACT_PATH.as_posix(), "fields must match the parsed decision metadata fields"))
    if contract.get("path_policy") != expected_path_policy:
        issues.append((INDEX_CONTRACT_PATH.as_posix(), "path_policy must match the full canonical-ID filename policy"))
    return issues
        ...
        if path.read_text(encoding="utf-8") != expected_text:
            issues.append(
                (
                    ...
                    "generated decision index is stale; run python scripts/docs_districts/generate_decision_indexes.py",
                )
            )
    return issues

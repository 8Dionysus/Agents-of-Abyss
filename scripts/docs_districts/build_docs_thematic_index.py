...
try:
    ...
    from scripts.docs_districts.docs_thematic_common import INDEX_REL, load_classifier, render_index, REPO_ROOT
def main()->int:
    ...
    if args.check:
        if not target.exists() or target.read_text(encoding='utf-8')!=rendered: raise SystemExit('generated docs thematic index is stale; run scripts/docs_districts/build_docs_thematic_index.py')

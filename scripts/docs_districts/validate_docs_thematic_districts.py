...
try:
    ...
    from scripts.docs_districts.docs_thematic_common import (
        CLASSIFIER_REL,
        DISTRICT_README_HEADINGS,
        REPO_ROOT,
        build_plan,
...
def require_validation_refs(repo_root: Path, classifier: dict) -> list[str]:
    ...
    release_check = repo_root / "scripts" / "release_gate" / "release_check.py"
    surfaces = [
    ...
    for command in classifier.get("validation_refs", []):
        ...
        if command.startswith("python ") and command not in release_commands:
            errors.append(f"scripts/release_gate/release_check.py missing docs guardrail command: {command}")
    return errors

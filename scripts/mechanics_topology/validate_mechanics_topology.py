...
def run_card_validator(selected: set[str] | None) -> list[str]:
    validator_path = REPO_ROOT / "scripts" / "mechanics_topology" / "validate_mechanic_readme_cards.py"
    spec = importlib.util.spec_from_file_location("validate_mechanic_readme_cards", validator_path)
    if spec is None or spec.loader is None:
        return ["cannot load scripts/mechanics_topology/validate_mechanic_readme_cards.py"]
...
def run_owner_request_validator(selected: set[str] | None) -> list[str]:
    validator_path = REPO_ROOT / "scripts" / "owner_requests" / "validate_owner_request_queue.py"
    if not validator_path.exists():
        return ["scripts/owner_requests/validate_owner_request_queue.py missing"]
    spec = importlib.util.spec_from_file_location("validate_owner_request_queue", validator_path)
    if spec is None or spec.loader is None:
        return ["cannot load scripts/owner_requests/validate_owner_request_queue.py"]
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
...
def run_owner_request_doc_validator(selected: set[str] | None) -> list[str]:
    validator_path = REPO_ROOT / "scripts" / "owner_requests" / "validate_owner_request_docs.py"
    if not validator_path.exists():
        return ["scripts/owner_requests/validate_owner_request_docs.py missing"]
    spec = importlib.util.spec_from_file_location("validate_owner_request_docs", validator_path)
    if spec is None or spec.loader is None:
        return ["cannot load scripts/owner_requests/validate_owner_request_docs.py"]
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)

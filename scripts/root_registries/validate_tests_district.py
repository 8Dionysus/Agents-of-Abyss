...
REGISTRY_PATH = Path("tests/registry.json")
SCHEMA_VERSION = "aoa_tests_registry_v1"
...
def validate_family(root: Path, family: dict[str, Any], seen_paths: dict[str, str]) -> list[str]:
    ...
    where = f"test_families[{family_id or '?'}]"
    ...
    require_string_list(problems, f"{where}.validation_refs", family.get("validation_refs"))
    ...
    if isinstance(paths, list):
        for rel in paths:
            if not isinstance(rel, str):
                continue
            if not rel.startswith("tests/test") or not rel.endswith(".py"):
                problems.append(f"{where}.paths entry must be a root test file: {rel}")
                continue
    ...
    validation_refs = family.get("validation_refs", [])
    if isinstance(validation_refs, list):
        for rel in validation_refs:
            if not isinstance(rel, str):
                continue
            if not rel.endswith(".py"):
                problems.append(f"{where}.validation_refs entry must be a Python file path: {rel}")
                continue
            if not rel_exists(root, rel):
                problems.append(f"{where}.validation_refs entry does not exist: {rel}")
...
def validate(root: Path) -> list[str]:
    ...
    if not registry_path.exists():
        return [f"missing tests registry: {REGISTRY_PATH}"]
    ...
    if registry.get("authority_ref") != "tests/README.md":
        problems.append("authority_ref must be tests/README.md")
    if registry.get("agents_ref") != "tests/AGENTS.md":
        problems.append("agents_ref must be tests/AGENTS.md")
    ...
    if not isinstance(root_policy, dict):
        ...
        root_policy = {}
    if root_policy.get("root_test_glob") != "tests/test*.py":
        problems.append("root_policy.root_test_glob must be tests/test*.py")
    require_string(problems, "root_policy.root_role", root_policy.get("root_role"))

    families = registry.get("test_families")
    if not isinstance(families, list) or not families:
        problems.append("test_families must be a non-empty list")
        families = []
    ...
    for family in families:
        if not isinstance(family, dict):
            problems.append("test_families entries must be objects")
            continue
        family_id = family.get("family_id")
    ...
    if registered != discovered:
        ...
        if missing:
            problems.append(f"root tests missing from registry: {', '.join(missing)}")
        if stale:
            problems.append(f"registry paths missing from tests/: {', '.join(stale)}")

    homes = registry.get("test_home_globs")
    if not isinstance(homes, list) or not homes:
        problems.append("test_home_globs must be a non-empty list")
        homes = []
    home_refs: set[str] = set()
    for home in homes:
        if not isinstance(home, dict):
            problems.append("test_home_globs entries must be objects")
            continue
        home_ref = home.get("home_ref")
        where = f"test_home_globs[{home_ref or '?'}]"
        ...
        if isinstance(path_glob, str) and not [path for path in root.glob(path_glob) if path.is_dir()]:
            problems.append(f"{where}.path_glob matches no test homes: {path_glob}")
    ...
    if release_check.exists():
        ...
        if "scripts/root_registries/validate_tests_district.py" not in release_text:
            problems.append("release_check.py does not run scripts/root_registries/validate_tests_district.py")

    return problems


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the root tests district registry.")
    ...
    if problems:
        print("Tests district validation failed:")
        for problem in problems:
            print(f" - {problem}")
        return 1
    print("tests district validated")
    return 0

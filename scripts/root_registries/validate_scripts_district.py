...
SCHEMA_VERSION = "aoa_scripts_registry_v2"
    ...
    if isinstance(paths, list):
        for rel in paths:
            ...
            if not rel.startswith("scripts/") or not rel.endswith(".py"):
                problems.append(f"{where}.paths entry must be a scripts Python file: {rel}")
                continue
            if isinstance(family_dir, str) and not rel.startswith(f"scripts/{family_dir}/"):
                problems.append(f"{where}.paths entry must live under scripts/{family_dir}/: {rel}")
...
def validate(root: Path) -> list[str]:
    ...
    if root_policy.get("family_python_glob") != "scripts/*/*.py":
        problems.append("root_policy.family_python_glob must be scripts/*/*.py")
    require_string(problems, "root_policy.root_role", root_policy.get("root_role"))
    require_string(problems, "root_policy.root_python_policy", root_policy.get("root_python_policy"))
    ...
    if root_python:
        problems.append(f"root-level scripts Python files are no longer allowed: {', '.join(root_python)}")
    ...
    if registered != discovered:
        ...
        if missing:
            problems.append(f"family scripts missing from registry: {', '.join(missing)}")
        if stale:
    ...
    if release_check.exists():
        ...
        if "scripts/root_registries/validate_scripts_district.py" not in release_text:
            problems.append("release_check.py does not run scripts/root_registries/validate_scripts_district.py")

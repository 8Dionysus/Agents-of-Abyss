...
AGENTS_MESH_SCRIPTS = REPO_ROOT / "scripts" / "agents_mesh"
sys.path.insert(0, str(AGENTS_MESH_SCRIPTS))
...
RPG_SOURCE_BOUNDARY_PATH = REPO_ROOT / "mechanics" / "rpg" / "parts" / "source-boundary" / "README.md"
RPG_CANONICAL_TERMINOLOGY_PATH = REPO_ROOT / "mechanics" / "rpg" / "parts" / "vocabulary-overlay" / "TERMINOLOGY.md"
DUAL_VOCABULARY_SCHEMA_PATH = REPO_ROOT / "mechanics" / "rpg" / "parts" / "vocabulary-overlay" / "schemas" / "dual_vocabulary_overlay.schema.json"
DUAL_VOCABULARY_EXAMPLE_PATH = REPO_ROOT / "mechanics" / "rpg" / "parts" / "vocabulary-overlay" / "examples" / "dual_vocabulary_overlay.example.json"
DUAL_VOCABULARY_GENERATED_PATH = REPO_ROOT / "mechanics" / "rpg" / "parts" / "vocabulary-overlay" / "generated" / "dual_vocabulary_overlay.json"
RPG_QUEST_CAMPAIGN_PATH = REPO_ROOT / "mechanics" / "rpg" / "parts" / "quest-campaign" / "README.md"
RPG_RUNTIME_PROJECTION_PATH = REPO_ROOT / "mechanics" / "rpg" / "parts" / "runtime-projection" / "README.md"
...
def validate_dual_vocabulary_generated_payload(payload: object, schema: dict[str, object]) -> None:
    if not isinstance(payload, dict):
        fail("mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json must be a JSON object")

    required_top_level = schema.get("required")
    if not isinstance(required_top_level, list):
        fail("mechanics/rpg/parts/vocabulary-overlay/schemas/dual_vocabulary_overlay.schema.json must declare top-level required fields")
    missing_top_level = [key for key in required_top_level if key not in payload]
    if missing_top_level:
        fail(
            "mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json is missing required keys: "
            + ", ".join(missing_top_level)
        )

    properties = schema.get("properties")
    if not isinstance(properties, dict):
        fail("mechanics/rpg/parts/vocabulary-overlay/schemas/dual_vocabulary_overlay.schema.json must declare properties")
    ...
    for field_name in ("overlay_id", "theme_id", "language"):
        ...
        if not isinstance(value, str) or not value:
            fail(f"mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json {field_name} must be a non-empty string")

    if payload.get("schema_version") != "dual_vocabulary_overlay_v1":
        fail("mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json schema_version must equal 'dual_vocabulary_overlay_v1'")
    if payload.get("public_safe") is not True:
        fail("mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json public_safe must be true")

    entries = payload.get("entries")
    if not isinstance(entries, list):
        fail("mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json entries must be a list")

    min_items = entries_schema.get("minItems")
    if isinstance(min_items, int) and len(entries) < min_items:
        fail(f"mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json entries must contain at least {min_items} items")
    max_items = entries_schema.get("maxItems")
    if isinstance(max_items, int) and len(entries) > max_items:
        fail(f"mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json entries must contain at most {max_items} items")

    item_schema = entries_schema.get("items")
    if not isinstance(item_schema, dict):
        fail("mechanics/rpg/parts/vocabulary-overlay/schemas/dual_vocabulary_overlay.schema.json entries.items must be an object")
    required_entry_fields = item_schema.get("required")
    if not isinstance(required_entry_fields, list):
        fail("mechanics/rpg/parts/vocabulary-overlay/schemas/dual_vocabulary_overlay.schema.json entries.items must declare required fields")
    item_properties = item_schema.get("properties")
    if not isinstance(item_properties, dict):
        fail("mechanics/rpg/parts/vocabulary-overlay/schemas/dual_vocabulary_overlay.schema.json entries.items must declare properties")
    ...
    if not isinstance(presentation_group_schema, dict) or not isinstance(
        ...
        fail("mechanics/rpg/parts/vocabulary-overlay/schemas/dual_vocabulary_overlay.schema.json must constrain presentation_group")
    ...
    for index, entry in enumerate(entries):
        if not isinstance(entry, dict):
            fail(f"mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json entries[{index}] must be an object")
    ...
    if duplicate_canonical_keys:
        fail(
            "mechanics/rpg/parts/vocabulary-overlay/generated/dual_vocabulary_overlay.json must not duplicate canonical_key values "
...
def validate_questbook_surface() -> None:
    ...
    if "AOA-Q-0006" in actual_ids:
        ...
        if "the machine vocabulary stays stable" not in canonical_terms_text:
            fail("mechanics/rpg/parts/vocabulary-overlay/TERMINOLOGY.md must keep machine vocabulary stability explicit")
        ...
        if schema_payload.get("title") != "dual_vocabulary_overlay_v1":
            fail("mechanics/rpg/parts/vocabulary-overlay/schemas/dual_vocabulary_overlay.schema.json title must equal 'dual_vocabulary_overlay_v1'")

        example_payload = read_json(DUAL_VOCABULARY_EXAMPLE_PATH)
        if not isinstance(example_payload, dict):
            fail("mechanics/rpg/parts/vocabulary-overlay/examples/dual_vocabulary_overlay.example.json must be a JSON object")
        if example_payload.get("schema_version") != "dual_vocabulary_overlay_v1":
            fail("mechanics/rpg/parts/vocabulary-overlay/examples/dual_vocabulary_overlay.example.json schema_version must equal 'dual_vocabulary_overlay_v1'")
        if example_payload.get("public_safe") is not True:
            fail("mechanics/rpg/parts/vocabulary-overlay/examples/dual_vocabulary_overlay.example.json public_safe must be true")

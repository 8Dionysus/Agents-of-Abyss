...
def validate(root: Path) -> list[str]:
    ...
    if registered_paths != discovered_paths:
        ...
        if stale:
            problems.append(f"registry scenario paths missing from .agents/spark/scenarios/: {', '.join(stale)}")
    ...
    problems.extend(validate_packet_dir(root, root / ".agents/spark/results", REQUIRED_RESULT_MARKERS, seen_ids))
    problems.extend(validate_packet_dir(root, root / ".agents/spark/handoffs/open", REQUIRED_HANDOFF_MARKERS, seen_ids))
    problems.extend(validate_packet_dir(root, root / ".agents/spark/handoffs/closed", REQUIRED_HANDOFF_MARKERS, seen_ids))

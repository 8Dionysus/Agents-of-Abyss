...
from scripts.docs_districts.docs_thematic_common import load_classifier, render_index
class DocsThematicIndexTests(unittest.TestCase):
    ...
    def test_index_is_machine_compact(self):
        text=(ROOT/'generated/docs_thematic_index.min.json').read_text(encoding='utf-8'); self.assertEqual(text.count('\n'),1); self.assertEqual(json.loads(text)['generated_rule'],'Reflects docs/guardrails/thematic_districts.json; does not author meaning.')

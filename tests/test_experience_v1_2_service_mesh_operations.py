...
def load_validator():
    path = ROOT / "scripts" / "validate_experience_v1_2_service_mesh_operations.py"
...
def load_example() -> dict[str, object]:
    return json.loads(
        (ROOT / "examples" / "experience_v1_2_service_mesh_operations.example.json").read_text(
            encoding="utf-8"
        )
    )
...
def load_schema() -> dict[str, object]:
    return json.loads(
        (ROOT / "schemas" / "experience-v1-2-service-mesh-operations.schema.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_experience_v12_service_mesh_validator_passes() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/validate_experience_v1_2_service_mesh_operations.py"],
        cwd=ROOT,
        text=True,
        capture_output=True,
    )

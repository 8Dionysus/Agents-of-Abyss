...
def load_validator():
    path = ROOT / "scripts" / "validate_experience_v1_3_office_foundry_role_pairs.py"
...
def load_example() -> dict[str, object]:
    return json.loads(
        (ROOT / "examples" / "experience_v1_3_office_foundry_role_pairs.example.json").read_text(
            encoding="utf-8"
        )
    )
...
def load_schema() -> dict[str, object]:
    return json.loads(
        (ROOT / "schemas" / "experience-v1-3-office-foundry-role-pairs.schema.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_experience_v13_office_foundry_validator_passes() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/validate_experience_v1_3_office_foundry_role_pairs.py"],
        cwd=ROOT,
        text=True,
        capture_output=True,
    )

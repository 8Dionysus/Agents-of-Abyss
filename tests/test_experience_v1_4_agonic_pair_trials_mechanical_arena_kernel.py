...
def load_validator():
    path = ROOT / "scripts" / "validate_experience_v1_4_agonic_pair_trials_mechanical_arena_kernel.py"
...
def test_experience_v14_mechanical_arena_validator_passes() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/validate_experience_v1_4_agonic_pair_trials_mechanical_arena_kernel.py"],
        cwd=ROOT,
        text=True,
        capture_output=True,
    )
...
def test_experience_v14_mechanical_arena_requires_current_agon_predecessors() -> None:
    ...
    predecessors.remove("docs/AGON_DUEL_KERNEL_MODEL.md")

...
class HygieneIndexTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for rel in ["config", "docs", "generated", "scripts/hygiene"]:
        ...
        for script in ["hygiene_common.py", "build_link_shape_hygiene_index.py", "validate_link_shape_hygiene_index.py"]:
            shutil.copy(repo / "scripts" / "hygiene" / script, self.tempdir / "scripts" / "hygiene" / script)
    ...
    def test_build_and_validate(self) -> None:
        build = subprocess.run([sys.executable, "scripts/hygiene/build_link_shape_hygiene_index.py"], cwd=self.tempdir)
        self.assertEqual(build.returncode, 0)
        check = subprocess.run([sys.executable, "scripts/hygiene/build_link_shape_hygiene_index.py", "--check"], cwd=self.tempdir)
        self.assertEqual(check.returncode, 0)
        valid = subprocess.run([sys.executable, "scripts/hygiene/validate_link_shape_hygiene_index.py"], cwd=self.tempdir)

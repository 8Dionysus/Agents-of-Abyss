...
class KnownRepairTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        shutil.copy(repo_scripts / "hygiene" / "repair_known_link_drifts.py", scripts / "hygiene" / "repair_known_link_drifts.py")

    def tearDown(self) -> None:
        shutil.rmtree(self.tempdir)

    def test_check_fails_then_apply_passes(self) -> None:
        bad = subprocess.run([sys.executable, "scripts/hygiene/repair_known_link_drifts.py", "--check"], cwd=self.tempdir, text=True, capture_output=True)
        self.assertNotEqual(bad.returncode, 0)
        apply = subprocess.run([sys.executable, "scripts/hygiene/repair_known_link_drifts.py", "--apply"], cwd=self.tempdir)
        self.assertEqual(apply.returncode, 0)
        ok = subprocess.run([sys.executable, "scripts/hygiene/repair_known_link_drifts.py", "--check"], cwd=self.tempdir)

...
When continuity fails across those seams, [RECURRENCE_PRINCIPLE](../mechanics/recurrence/docs/RECURRENCE_PRINCIPLE.md) governs the bounded return to a valid anchor without transferring source ownership to routing, infrastructure, or derived layers.
...
When KAG counterpart work grows, the bridge doctrine in `mechanics/boundary-bridge/parts/counterpart-edge/README.md` and `mechanics/boundary-bridge/docs/COUNTERPART_BRIDGE.md` should stay the governing policy.

...
| `Dionysus` | `dionysus_personal_portrait_projection_bundle`, `public_media_export` | public protocol documents and purpose-bounded portrait projection/media/docs export | light ABI and provenance for a generated projection bundle; C2PA for public media/docs exports; no ABI or public bundle for private raw interview material | SLSA/Cosign only for published release assets; SBOM only for packaged software | agent, release consumer | owns interview, consent, evidence, claim, review, and portrait-projection meaning; private raw material stays outside Git |
| `aoa-sdk` | `aoa_sdk_python_distribution`, `thin_routing_readmodel_bundle` | package/distribution, typed trust-plane access, and SDK-canonical routing readmodels | ABI, SBOM, SLSA/in-toto for wheel/sdist, release bundle, and routing readmodel | Cosign for release assets; TUF for update/install clients; no C2PA | agent, installer/update client | owns typed readers/assertions and routing/dispatch contracts, not host blocking decisions, runtime execution, or source meaning |
...
| `aoa-routing` predecessor | historical `thin_routing_readmodel_bundle` receipts only | deprecated routing artifacts retained for audit and rollback | preserve digest, provenance, and prior validation receipts; no new producer admission | no new signing or publication lane; archive remains separately operator-controlled | auditor, rollback reviewer | owns no new navigation or dispatch authority after succession |
